...
def create_bar(df, output_tag):
    ...
    plt.tight_layout()
    plt.savefig(output_tag + "_bar_graph.png")
...
def create_pie(df, output_tag):
    ...
    plt.ylabel('')
    plt.tight_layout()
    plt.savefig(output_tag + "_pie_chart.png")
...
def create_scatter(df, output_tag):
    ...
    plt.ylabel("Counts")
    plt.tight_layout()
    plt.savefig(output_tag + "_scatter_plot.png")
...
def visualize_data(json_file, output_tag):
    plt.clf()
    df = pd.DataFrame()
    with open(json_file, 'r') as f:
        df = pd.read_json(f)
    create_scatter(df, output_tag)
