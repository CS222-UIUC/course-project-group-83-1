...
"""
Main function, combines everything together
    input:
        [str] json_input   -> json file of scraped tweets
        [list] tags        -> list of strings, ordered: [tweet_col, polarity_col, subjectivity_col]
        [str] output       -> filename (including extension) for the fully processed polarity tweets
        [str] avg_output   -> OPTIONAL - filename (including extension) for averaged polarities
    output:
        [Dataframe] df     -> pd.DataFrame object with calculated polarities + subjectivity
"""
def main_process_polarity(json_input, tags, output, avg_output=""):
